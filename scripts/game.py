...
class Field():
    ...
    def check_win(self) -> bool:
        if py.is_key_down(py.KeyboardKey.KEY_W):
            self.won = True
            return True
...
class Game():
    ...
    def render(self) -> None:
        ...
        if self.game_field.won != None:
            self.render_win_anim(self.game_field.won)

    def start_win_anim(self, color: bool) -> None:
        self.last_time = time.time()

    def play_win_anim(self, color: bool) -> None:
        self.delta_time = (time.time() - self.last_time) * 24
        self.last_time = time.time()

        self.text_pos += 25 * self.delta_time

    def render_win_anim(self, color: bool) -> None:
        py.draw_rectangle(0, 0, 603, 603, py.Color(0, 0, 0, 100))

        if color == self.color:
            py.draw_text("You've Won!", int(self.text_pos), 200, 60, py.RAYWHITE)

        else:
            py.draw_text("You've Lost!", int(self.text_pos), 200, 60, py.RAYWHITE)

        py.draw_rectangle(603, 0, 297, 603, py.BLACK)
    ...
    def update(self, conn, profile: Profile) -> None:
        ...
        if self.game_field.won == None:
            if self.game_field.update():
                self.start_win_anim(self.game_field.won)
            ...
            self.play_win_anim(self.game_field.won)
            if self.text_pos > 603:
                self.finished = True
